    <x-form metod="POST" action="{{ route('reginfo.update') }}" id="regform">
        ...
        <x-input hidden name="name" id="name" value="{{ Auth::user()->regname }}" />
        ...
        <x-bladewind::card title="Personal Information" has_shadow="true">
            <div class="flex flex-col columns-1 md:columns-2 lg:columns-4">
                ...
                <x-bladewind::checkbox label="I am a member of engineering society" name="engsociety" value="1"
                    :checked="$registrant->engsociety == 1" />
            </div>
        </x-bladewind::card>
        ...
        <x-bladewind::centered-content size="tiny">
            <x-bladewind::button type="primary" name="command" icon="server"
                can_submit="true">{{ __('Save') }}</x-bladewind::button>
