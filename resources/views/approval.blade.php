                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-800 dark:text-gray-100">
                        <div class="flex flex-col">
                            ...
                            <form  method="post" :action="url('/approveorder/'.$order->id)">
                                @csrf
                                @method('post')
                            <div class="grid grid-cols-1 lg:grid-cols-4 md:grid-cols-2 gap-1 m-3 p-3">
                                <x-label for="commitee_meeting_number" />
                                <x-bladewind::input name="meetingnumber" id="meetingnumber" />
                                <x-label for="commitee_meeting_date" />
                                <x-bladewind::datepicker name="meetingdate" id="meetingdate" />
                                <x-label for="approval_Status" />
                                <x-bladewind::radio-button name="approval" color="green" id="ckyes" label="Approved" />
                                <x-bladewind::radio-button name="approval" id="ckno" color="red" label="Rejected" />
                            </div>
                            <div>
                            ...
                            <div class="flex flex-grow">
                                <x-bladewind::button color="green" icon="check" tag="button"
                                   can_submit="true">
                                    {{ __('Save') }}
                                </x-bladewind::button>
